    ...
    <div class="admin-right">
        <div class="admin-right-container">
            ...
            <form class="mt16 flex flex-vc flex-tj" @submit.prevent="filter()">
                <div class="flex flex-vc">
                    <div class="select-box mr10">
                        <select v-model="form.state" @change="() => {form.page=1; filter();}">
                ...
                <table>
                    ...
                    <tr v-for="item in items.data" :key="item.id">
                        ...
                        <td style="text-align: left;">
                            <span class="primary" v-if="!item.answer">[답변대기]</span> {{ item.title }}
                        </td>
                        <td>{{ item.created_at }}</td>
                        <td>
                            <div class="table-button-box">
                                <nuxt-link :to="`/admin/qnas/create?id=${item.id}`" class="active">조회</nuxt-link>
            ...
            <div class="flex flex-vc flex-tj mt32">
                ...
                <div class="button-box">
                    <nuxt-link to="/admin/qnas/create" class="btn btn-blue px45">등록</nuxt-link>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        remove(item){
            ...
            this.$axios.delete("/api/admin/qnas/" + item.id, {
                params: this.form.data()
            }).then(response => {
