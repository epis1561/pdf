                    ...
                    <tr v-for="item in items.data" :key="item.id">
                        ...
                        <td>
                            <div class="table-button-box">
                                <nuxt-link :to="`/admin/emails/create?id=${item.id}`" class="active">조회</nuxt-link>
<!--                                <a href="#" @click.prevent="remove(item)">삭제</a>-->
            ...
            <div class="flex flex-vc flex-tj mt32">
                ...
                <div class="button-box">
                    <nuxt-link to="/admin/emails/create" class="btn btn-blue px45">등록</nuxt-link>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        filter(){
            ...
            this.$axios.get("/api/admin/emails", {
        ...
        remove(item){
            ...
            if(confirmed)
                this.form.delete("/api/admin/emails/" + item.id)
                    .then(response => {
        ...
        up(item){
            ...
            if (index > 0) {
                ...
                this.form.patch("/api/admin/emails/" + item.id + "/up");
            }
        },
        ...
        down(item){
            ...
            if (index < this.items.data.length - 1) {
                ...
                this.form.patch("/api/admin/emails/" + item.id + "/down");
        ...
        importExcel(e){
            ...
            this.form.post("/api/admin/emails/import")
                    .then(response => {
        ...
        exportExcel(){
            ...
            this.form.post("/api/admin/emails/export")
                    .then(response => {
                        if(response.data){
                            let a = document.getElementById("download");
